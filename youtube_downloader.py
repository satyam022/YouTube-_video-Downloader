...
class Youtube:
    ...
    def show_yt_logo(self):
        ...
        image = Image.open('images/youtube.png')
        # Resizing the image
        resized_image = image.resize((190, 110))
    ...
    # The `Paste_URL` method retrieves the URL copied to the clipboard and inserts it into the Entry Box, utilising the Python `pyperclip` module.
        ...
        if self.URL.get() == '':
            ...
            try:
                ...
                if self.quality_combobox.get() == 'Audio Only':
                    ...
                    self.status.config(text="Downloading...")
                    video = yt.streams.filter(mime_type="video/mp4", res=self.quality_combobox.get(),
...
if __name__ == "__main__":
    root = Tk()
    # Creating a 'Youtube' class object
    obj = Youtube(root)
    root.mainloop()
